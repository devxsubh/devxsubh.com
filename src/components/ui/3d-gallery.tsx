...
function ImagePlane({
	...
	return React.createElement(
		...
		React.createElement('planeGeometry', { args: [1, 1, 32, 32] })
	);
}
...
function GalleryScene({
	...
	onImageClick,
}: Omit<InfiniteGalleryProps, 'className' | 'style'> & { onImageClick: (imageData: { src: string; alt: string }) => void }) {
				...
				return (
					<ImagePlane
						...
						imageData={{
							...
							alt: normalizedImages[plane.imageIndex].alt || 'Gallery Image'
						}}
					/>
				);
			})}
...
export default function InfiniteGallery({
	...
	const handleImageClick = useCallback((imageData: { src: string; alt: string }) => {
			...
			{selectedImage && (
				...
				>
					<div className="relative max-w-[80vw] max-h-[80vh] w-full h-full flex items-center justify-center">
						...
						<button
							onClick={closeModal}
							className="absolute top-4 right-4 w-10 h-10 bg-white/20 hover:bg-white/30 rounded-full flex items-center justify-center text-white text-2xl font-bold transition-colors backdrop-blur-sm"
						>
							×
						</button>
						<div className="absolute bottom-2 sm:bottom-4 left-2 right-2 sm:left-1/2 sm:right-auto sm:transform sm:-translate-x-1/2 text-center">
							<p className="text-white/90 text-xs sm:text-sm bg-black/60 px-2 sm:px-4 py-1 sm:py-2 rounded-full inline-block backdrop-blur-sm max-w-full break-words">
								{selectedImage.alt}
							</p>
						</div>
					</div>
